...
const SalesExecutive = () => {
  ...
  const fetchCounts = useCallback(async () => {
    const [customerResponse, reviewResponse] = await Promise.all([
      fetch('https://65.1.180.81:8000/sales/customers/count', {
        method: 'GET',
        headers: { accept: 'application/json', Authorization: `Bearer ${token}` }
      }),
      fetch('https://65.1.180.81:8000/sales/customer-verification/count', {
        method: 'GET',
        headers: { accept: 'application/json', Authorization: `Bearer ${token}` }
      }),
    ]);
  ...
  const fetchCustomers = useCallback(async () => {
    const response = await fetch('https://65.1.180.81:8000/sales/customers', {
      method: 'GET',
      headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await fetch('https://65.1.180.81:8000/sales/create-customer', {
            method: 'POST',
            headers: {
