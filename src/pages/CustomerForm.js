...
const CustomerForm = () => {
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        const response = await fetch(`https://65.1.180.81:8000/customer/customer-form/${link_token}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://65.1.180.81:8000/customer/${link_token}`, {
        method: 'POST',
        body: formDataToSend,
      });
