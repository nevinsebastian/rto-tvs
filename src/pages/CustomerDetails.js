...
const CustomerDetails = () => {
  ...
  useEffect(() => {
    const fetchCustomerById = async () => {
      try {
        const response = await fetch(`https://65.1.180.81:8000/sales/customers/${customerId}`, {
          method: 'GET',
          headers: {
  ...
  const handleSaveClick = async () => {
    ...
    try {
      const response = await fetch(`https://65.1.180.81:8000/sales/customers/delivery-update/${customerId}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  const handleVerifyCustomer = async () => {
    try {
      const response = await fetch(`https://65.1.180.81:8000/sales/verify/${customerId}`, {
        method: 'POST',
        headers: {
          accept: 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`,
